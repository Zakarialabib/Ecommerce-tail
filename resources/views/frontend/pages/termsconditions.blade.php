...
@section('title','Terms and Conditions || leMotoShop')
    ...
    <div class="container">
        <div class="breadcrumb-content text-center">
            <ul>
                ...
                <li class="active"><a
                        href="javascript:void(0)">{{ __('Terms and Conditions') }}</a></li>
            </ul>
        </div>
    </div>
</div>
                    ...
                    @foreach($faqs as $key => $faq)
                        <div class="panel">
                            <div class="panel-heading">
                                <h4 data-toggle="collapse" aria-expanded="@if ($loop->first) true @endif"
                                    data-target="#id{{ $faq->id }}" aria-controls="id{{ $faq->id }}"
                                    class="panel-title">
                                    {{ $faq->title }}
                                </h4>
                            </div>
                            <div id="id{{ $faq->id }}" class="panel-collapse collapse @if ($loop->first) show @endif"
                                aria-labelledby="id{{ $faq->id }}" data-parent="#accordion-1">
