...
<div class="card">
    <h5 class="card-header">{{ __('Edit Currency') }}</h5>
    <div class="card-body">
        <form class="form-horizontal" action="{{ route('currency.update',$currency->id) }}"
            method="POST">
            @csrf
            ...
            <div class="form-group">
                ...
                <div class="sm:w-4/5 pr-4 pl-4">
                    <input type="text" step="0.1" min="0" class="form-control" name="value"
                        placeholder="{{ __('Value') }}" value="{{ $currency->value }}">
                    @if($errors->has('value'))
